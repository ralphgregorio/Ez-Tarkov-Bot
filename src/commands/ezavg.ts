...
export default new Command({
    name: "ezavg",
    description: "Shows the top 5 items with the biggest flips based on their 24h average price",
    run: async ({ interaction }) => {
        ...
        const profitability: ItemProfitability[] = items.map( (item: Item) => {
            ...
            if ( !item || !sellTo || !item.avg24hPrice || !item.sellFor) {
                return  {
                    name: item?.name,
                    sellingSource: 'N/A',
                    basePrice: '0',
            ...
            const profit = item.avg24hPrice ? biggestTraderSellValue - item.avg24hPrice: 0;
